...
export const inngest = new Inngest({ id: "projectly.dev" });
...
const syncWorkspaceCreation = inngest.createFunction(
  ...
  async ({ event }) => {
    const { data } = event
    console.log("🟢 [syncWorkspaceCreation] Event data:", data);
        ...
        if (task.status !== 'DONE') {
          await step.run('send-task-reminder-mail', async () => {
            await sendEmail({
              ...
              body: `<div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; background: #ffffff; border-radius: 8px; padding: 20px; border: 1px solid #eee;">
                <h2>Hi ${task.assignee.name}, 👋</h2>
...
export const functions = [syncUserCreation, syncUserDeletion, syncUserUpdation, syncWorkspaceCreation, syncWorkspaceDeletion, syncWorkspaceUpdation, syncWorkspaceMemberCreation, sendTaskAssignmentEmail];